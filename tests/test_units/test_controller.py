...
class BasicWSGIController(WSGIController):
    ...
    def params(self):
        items = list(pylons.request.params.mixed().items())
        items.sort()
        return str(items)
...
class TestBasicWSGI(TestWSGIController):
    ...
    def test_unicode_action(self):
        self.baseenviron['pylons.routes_dict']['action'] = 'ÐžÐ±ÑÑƒÐ¶Ð´ÐµÐ½Ð¸ÐµÐšÐ¾Ð¼Ð¿Ð°Ð½Ð¸Ð¹'
        resp = self.app.get('/', status=404)

    def test_params(self):
        self.baseenviron['pylons.routes_dict']['action'] = 'params'
