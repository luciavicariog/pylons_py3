...
def make_basexmlrpc():
    ...
    class BaseXMLRPCController(XMLRPCController):
        ...
        def uni(self):
            "This method has a docstring"
            return dict(mess='A unicode string, oh boy')
        uni.signature = [ ['struct'] ]
    
        def intargcheck(self, arg):
            if not isinstance(arg, int):
                return xmlrpc.client.Fault(0, 'Integer required')
            else:
                return "received int"
...
class TestXMLRPCController(TestWSGIController):
    ...
    def test_unicode_method(self):
        data = xmlrpc.client.dumps((), methodname='ОбсуждениеКомпаний')
    ...
    def test_badargs(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'system.methodHelp')
    ...
    def test_bad_paramval(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'intargcheck', (12.5,))
    
    def test_missingmethod(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'doesntexist')
    ...
    def test_nosignature_unicode(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'system.methodSignature',
                          ('ОбсуждениеКомпаний',))
    ...
    def test_start_response(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'start_response')
    
    def test_private_func(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, '_private')
    
    def test_var(self):
        self.assertRaises(xmlrpc.client.Fault, self.xmlreq, 'foo')
